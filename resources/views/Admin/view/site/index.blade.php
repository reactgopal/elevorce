            ...
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="example" class="table table-striped text-center text-dark" style="min-width: 1000px;">
                            ...
                            <tbody>
                                @forelse ($sites as $key => $site)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ url('admin/site-manager-list/'.$site->id) }}" class="btn btn-sm btn-warning" title="View Site Manager">
                                                    <i class="fas fa-user-tie"></i>
                                                </a>
                                            <a href="{{ url('admin/employee-list/'.$site->id) }}" class="btn btn-sm btn-info" title="View Employees">
                                                    <i class="fas fa-users"></i>
                                                </a>
                                            <a href="{{ route('site.show', $site->id) }}" class="btn btn-sm btn-warning" title="View">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ url('admin/edit/'.$site->id.'/'.$employer_id) }}" class="btn btn-sm btn-primary" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('site.destroy', $site->id) }}" method="POST"
                                                  style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this?')">
                                                @csrf
                                                @method('DELETE')
